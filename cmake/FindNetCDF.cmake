...
set( NetCDF_C_PKGCONFIG_NAME        netcdf )
set( NetCDF_CXX_PKGCONFIG_NAME      netcdf-cxx4 )
set( NetCDF_Fortran_PKGCONFIG_NAME  netcdf-fortran )
...
foreach( _comp ${_search_components} )
  ...
  ## Try pkgconfig
  if ( PkgConfig_FOUND AND ( _comp STREQUAL "C" OR _comp STREQUAL "CXX" OR _comp STREQUAL "Fortran" ))
    pkg_search_module(_NetCDF_${_comp} QUIET ${NetCDF_${_comp}_PKGCONFIG_NAME} IMPORTED_TARGET)
    set( _NetCDF_${_comp}_CMAKE_TARGET "PkgConfig::_NetCDF_${_comp}" )
    if ( NOT NetCDF_VERSION )
      set( NetCDF_VERSION ${_NetCDF_${_comp}_VERSION} )
    endif ()
  endif ()
  if ( _NetCDF_${_comp}_FOUND )
    set( ${CMAKE_FIND_PACKAGE_NAME}_${_arg_${_COMP}}_FOUND TRUE )
    set( _found TRUE )
    mark_as_advanced(_NetCDF_${_comp}_CMAKE_TARGET)
    list( GET _NetCDF_${_comp}_INCLUDE_DIRS 0 NetCDF_${_comp}_INCLUDE_DIR )
    mark_as_advanced(_NetCDF_${_comp}_INCLUDE_DIRS NetCDF_${_comp}_INCLUDE_DIR)
    get_target_property(_NetCDF_${_comp}_LIBRARIES ${_NetCDF_${_comp}_CMAKE_TARGET} INTERFACE_LINK_LIBRARIES)
    list( GET _NetCDF_${_comp}_LIBRARIES 0 NetCDF_${_comp}_LIBRARY )
    mark_as_advanced(_NetCDF_${_comp}_LIBRARIES NetCDF_${_comp}_LIBRARY)
    if ( NOT TARGET NetCDF::NetCDF_${_comp} )
      ...
      set_target_properties(NetCDF::NetCDF_${_comp} PROPERTIES
        INTERFACE_LINK_LIBRARIES ${_NetCDF_${_comp}_CMAKE_TARGET})
...
## Find version
if (_found AND NOT NetCDF_VERSION)
